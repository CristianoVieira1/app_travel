...
class BelemMaps extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => BelemMapsState();
}

class BelemMapsState extends State<BelemMaps> {
  final List<MapDestination> _destinations = [
    MapDestination("Praia do Leblon", _kLondonEye),
  ];
  ...
  static final CameraPosition _kLondonEye =
      CameraPosition(target: LatLng(-30.2097508, -51.1871446), zoom: 14.4746);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Praia do Leblon"),
        backgroundColor: Colors.lightBlueAccent,
        leading: IconButton(
          icon: Icon(FontAwesomeIcons.arrowLeft),
          color: Colors.white, onPressed: () {
            Navigator.of(context).pushReplacementNamed('/belem');
          },
        ),
        actions: <Widget>[
          Container(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => MapsLauncher.launchCoordinates(
                    -30.2097508, -51.1871446, 'Praia do Leblon'),
