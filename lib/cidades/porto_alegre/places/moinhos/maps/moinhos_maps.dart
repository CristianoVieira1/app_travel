...
class MoinhosMaps extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => MoinhosMapsState();
}

class MoinhosMapsState extends State<MoinhosMaps> {
  final List<MapDestination> _destinations = [
    MapDestination("Parque Moinhos de Vento", _kLondonEye),
  ];
  ...
  static final CameraPosition _kLondonEye =
      CameraPosition(target: LatLng(-30.027267, -51.2025817), zoom: 14.4746);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Parque Moinhos de Vento"),
        backgroundColor: Colors.blue,
        leading: IconButton(
          icon: Icon(FontAwesomeIcons.arrowLeft),
          color: Colors.white, onPressed: () {
            Navigator.of(context).pushReplacementNamed('/moinhos');
          },
        ),
        actions: <Widget>[
          Container(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => MapsLauncher.launchCoordinates(
                    -30.027267, -51.2025817, 'Parque Moinhos de Vento'),
