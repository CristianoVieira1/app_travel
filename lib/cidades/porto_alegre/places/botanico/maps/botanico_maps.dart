...
class BotanicoMaps extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => BotanicoMapsState();
}

class BotanicoMapsState extends State<BotanicoMaps> {
  final List<MapDestination> _destinations = [
    MapDestination("Jardim Botânico", _kLondonEye),
  ];
  ...
  static final CameraPosition _kLondonEye =
      CameraPosition(target: LatLng(-30.051415, -51.177869), zoom: 14.4746);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Jardim Botânico"),
        backgroundColor: Colors.lightBlueAccent,
        leading: IconButton(
          icon: Icon(FontAwesomeIcons.arrowLeft),
          color: Colors.white, onPressed: () {
            Navigator.of(context).pushReplacementNamed('/botanico');
          },
        ),
        actions: <Widget>[
          Container(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => MapsLauncher.launchCoordinates(
                    -30.051415, -51.177869, 'Jardim Botânico'),
